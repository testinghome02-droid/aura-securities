  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-linear-to-b from-slate-950 via-slate-950 to-slate-900 text-white">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -left-20 top-24 h-64 w-64 rounded-full bg-brand-500/10 blur-3xl" />
        <div className="absolute right-8 top-64 h-72 w-72 rounded-full bg-fuchsia-500/10 blur-3xl" />
      </div>
      <section
        className="relative mx-auto w-full max-w-7xl space-y-6 px-6 py-16"
        aria-labelledby="mutual-fund-title"
      >
        <header className="space-y-4 rounded-3xl border border-slate-800 bg-linear-to-br from-slate-900/70 to-slate-950 p-8 shadow-lg shadow-slate-900/40">
          ...
          <h1 id="mutual-fund-title" className="text-4xl font-semibold">
            Grow with expert-managed funds
          </h1>
          <p className="text-slate-300">
            Invest in mutual fund to grow your wealth with expert-managed
            portfolios and diversified strategies.
          </p>
        </header>

        <ul className="grid gap-4 md:grid-cols-3">
          {["Balanced funds", "Growth funds", "Income funds"].map((item) => (
            <li
              key={item}
        ...
        <section className="grid gap-4 md:grid-cols-3">
          {[
            ...
            { label: "SIP Start", value: "₹500/month" },
            { label: "Expert Picks", value: "Curated" },
          ].map((stat) => (
            <div
              key={stat.label}
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-slate-300">
                Build wealth with flexible monthly investments guided by
                experts.
              </p>
            </div>
            <nav
